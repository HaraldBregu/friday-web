...
assert(/\.fr-extension-screen__workspace \{[\s\S]*?height: 100%;/.test(landingCss) && /\.fr-extension-workspace-ui,[\s\S]*?\.fr-extension-coding-ui \{[\s\S]*?height: 100%;/.test(landingCss), "every extension interface fills the full app window height");
