...
for (const [locale, html, title] of [
  ...
  assert(count(html, /class="fr-showcase-app fr-extension-screen is-/g) === 3, `${locale} renders Workspace, Design Agent, and Coding as distinct Friday app screens`);
  ...
  assert(html.includes('class="fr-extension-coding-ui__sidebar"') && !html.includes('class="fr-extension-coding-ui__editor"'), `${locale} renders the Coding agent sidebar without editor tabs`);
