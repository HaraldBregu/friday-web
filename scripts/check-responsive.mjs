...
for (const [locale, html, title] of [
  ...
  assert(count(html, /class="fr-showcase-history__item is-/g) === 3, `${locale} hero app includes earlier image, sound, and video requests`);
  assert(html.includes("video-1786447738614.mp4") && html.includes("video-1786619008446.mp4") && html.includes("data-showcase-play") && html.includes("data-showcase-progress"), `${locale} hero app uses both current demo videos with playable controls`);
