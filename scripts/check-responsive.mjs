...
for (const [locale, html, title] of [
  ...
  assert(count(html, /class="fr-showcase-history__item is-/g) === 2, `${locale} hero app includes earlier image and sound requests`);
  assert(html.includes("video-1784367922287.mp4") && html.includes("data-showcase-play") && html.includes("data-showcase-progress"), `${locale} hero app renders playable video controls`);
